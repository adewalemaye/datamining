...
# %% [markdown]
# ## Part 1
# ### Remember toggle between line wrap, (Option-Z on Mac, and Alt-Z on Windows?) to view longer lines. I like to keep some lines intact, to keep the overall structure easier to follow.
# 
# ### Let us try to use OOP to manage something useful.
# 
# ### This first part of the exercise, we will use just basic python and OOP to create a Stock class, to keep track of different stocks in a portfolio.Let's say, Apple. (NO numpy nor pandas here in this Part I.)
# 
# ### I pulled data from https://old.nasdaq.com/symbol/aapl/historical (5 years, csv format, 9/12/2019). You can also try from something like  www.quandl.com/EOD
# 
# ### The csv file is given to you here, along with Microsoft (MSFT) and Google (the parent company is Alphabet now, stock symbol GOOG) files. 
# 
...
# ### For example, from the daily data with only date, end-of-day closing price, and trading volumn as three columns, we want to be able to find the daily changes easily, and also the change of the last 50 days, 100 days, for example. The daily change represents the "first derivatives" in calculus or economics term. It is also very common to look at the "second derivatives", which is the "rate of change of the rate of change of the stock price". This "second derivative" represent something like a overall trend/momentum. A negative 2nd derivative means even though the stock price change (1st derivative) is positive, but the increase is slowing down. You might want to find a time to get out.
# 
...
# 
# ### below is other attributes that will be kept as a python list. When the object is initialized, it reads in the data file and calculate the rest
...
# 
#  
# ### Most of the codes are written for you. Your task is to understand them, and fill in the rest to make it work.
# 
# ### Let's start: 

# %% [markdown]
# ### Step 0, try reading the data file to make sure it works. If your data file is in the same folder, you don't need to put in the full path. Just the file name. 
# 
# ### Run this cell as is. If you see some output in the interactive python window, then it is working. If not, you might need to fix the file path accordingly for your OS/platform.You can use functions like os.getcwd() to find out what is the current working directory in your ipython session. If needed, you can use os.chdir( your path ) to set the current directory to where you need. like: filepath = "/Users/ningrui/GDrive_GWU/github_nr/GWU_classes_p/DATS_6103_DataMining/Assignment/AAPL_daily.csv"
...
def import_history(self, filepath):
    ...
    with open(filepath,'r') as fh: # leaving the filehandle inside the "with" clause will close it properly when done. Otherwise, remember to close it when finished
      for aline in fh.readlines(): # readlines creates a list of elements; each element is a line in the txt file, with an ending line return character.
          ...
          self.volumes.append(float(tmp[1]))
          self.price_eod.append(float(tmp[1]))  
    fh.close()
    print('fh closed:',fh.closed)
    return self
...
def compute_delta2_list(self):
    ...
    delta_shift1 = self.delta1.copy()
    delta_shift1.pop(0)
    self.delta2 = list(map(lambda x,y: x-y, self.price_eod, delta_shift1))
    print(self.name.upper(),": The latest 5 daily changes in delta1: ")
...
def insert_newday(self, newdate, newprice, newvolume):
    ...
    # Fill in the codes here 
    #
    self.dates.insert(self.newdate + self.dates)
    # insert newvolume to volumes[]
    self.volumes.insert(self.newvolume + self.volumes)
    # insert new eod data value to price_eod
    self.price_eod.insert(self.newprice + self.price_eod)
    # calculate and insert new data to delta1
    self.delta1.insert(self.delta1.new + self.delta1)
    # calculate and insert new data to delta2
    self.delta2.insert(self.delta2.new + self.delta2)
    #
...
def nday_change_percent(self,n):
    ...
    change = self.newprice - self.price_eod/self.price_eod * 100 # calculate the change of price between newest price and n days ago
    percent = self.newprice - n/n * 100  # calculate the percent change (using the price n days ago as the base)
...
# 
nday_change_percent(self: Any, n: 50)
nday_change_percent(self: Any n: 200)
nday_change_percent(self: Any, n: = 300)
